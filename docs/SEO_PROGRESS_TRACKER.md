...
| **Batch 3** | Clean Energy Trade Media Editors (Solar Power World, InsideEVs, EC&M, Canary, PV Mag) | **August 27, 2026** | **Completed (Sept 3)** | **Follow-ups Sent (Sept 3):** *Solar Power World* (`kannenberg@wtwhmedia.com`), *EC&M* (`eparson@endeavorb2b.com`), *Canary Media* (`editors@canarymedia.com`).<br>**Initial Pitch Sent (Sept 3):** *PV Magazine* (`tim.sylvia@pv-magazine.com`; follow-up due Sept 10).<br>⚠️ *Delivery Issue:* `tim.levin@insideevs.com` (not found/bounced); queued for alternative contact (`mark.kane@insideevs.com` / `contact@insideevs.com`). |
...
| **Batch 6** | Open Educational Resources & STEM Libraries (LibreTexts, MIT OCW, OER Commons, MERLOT, CC) | **September 3, 2026** | **September 10, 2026** | **Sent & Verified (Sept 3):** LibreTexts (`dlarsen@ucdavis.edu`), MIT OCW (`cnewton@mit.edu`), OER Commons (`barbara.soots@k12.wa.us`), MERLOT (`merlot@calstate.edu`), Cathy Casserly (`cathy@casserlyconsulting.com`). |
