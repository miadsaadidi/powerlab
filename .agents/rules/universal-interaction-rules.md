# Universal Interaction Protocol — PowerLab SEO Operating Model

**Scope:** Applicable across ALL conversations, domains, and tasks (coding, refactoring, SEO, outreach, documentation, UI, architecture).

---

## 🛑 Hard Circuit Breaker: Zero Execution on Questions (MANDATORY)

1. **Question Detection:** If the user prompt contains a question mark (`?`), asks for status (*"how is X"*), or asks for planning/next steps (*"what's next"*, *"what to do"*, *"what should we do"*, *"what's the plan"*):
...
* **Top Technical Priorities (5-Tier SEO Hierarchy):**
  1. **Tier 1 (Core Google Search Assets):** Sizing calculators, research whitepapers, benchmark datasets, technical guides, and methodology pages answering real search intent.
  2. **Tier 2 (Topic Cluster Internal Linking):** Crawlable, bidirectional links connecting Guide ↔ Calculator ↔ Dataset ↔ Research across topic clusters.
  3. **Tier 3 (Search-Intent & On-Page Substance):** Equations, worked examples, assumptions, standards compliance (NEC/IEEE/ASHRAE), and search-aligned headings.
  4. **Tier 4 (Technical SEO & Crawl Health):** Indexability, XML sitemaps, canonical tags, HTTP status codes, Core Web Vitals, and Schema.org structured data.
  5. **Tier 5 (Supporting External Distribution):** Distinct external technical articles, open repository DOIs, and educational courseware as supporting discovery/citation layers (zero quotas).
...
* Never fabricate impressions, clicks, CTR, rankings, crawl stats, backlinks, publications, or citations.
